...
document.addEventListener("DOMContentLoaded", () => {
	...
	$('#div_fecha_inicial').datetimepicker({
		...
		cargarCropsEntreLasFechas(
			...
			() => {
				cargarEmpleadosEntreLasFechasGranjaCrops(
					$('#initial_date').val(),
					$('#final_date').val(),
					$('#cod_farm_reporte_piece_rate').val(),
...
function generarReporte() {
	...
	$.ajax({
		type: "POST",
		// url: window.location.origin + "/public/api/v1/reportes/obtenerRateReportData",
		url: window.location.origin + "/api/v1/reportes/obtenerRateReportData",
		data: {
			employees: employees,
...
function cargarEmpleadosEntreLasFechasGranjaCrops(
	initial_date,
	final_date,
	cod_farm = [],
	cod_crop = [],
	...
	$.ajax({
		type: "POST",
		// url: window.location.origin + "/public/api/v1/empleado_administracion/searchEmpleadosPorGranjaCrop",
		url: window.location.origin + "/api/v1/empleado_administracion/searchEmpleadosPorGranjaCrop",
		data: {
		...
		let row = `
            <div class="col-12">
                <div class="row w-100 border-bottom">
                    ...
                    <div class="col-1 px-2 py-1">
                        <small>${fila.nombre_categoria}</small>
                    </div>
                    <div class="col-2 px-2 py-1">
                        <div>
                            <small>${fila.nombre_semilla}</small>
                        </div>
                        <small style="color:gray;font-size:0.8em">
                            Age: ${fila.edad}
                        </small>
                    ...
                    <div class="col-1 px-2 py-1">
                        <small>${fila.tipo_pack}</small>
                    </div>
                    <div class="col-1 px-2 py-1">
                        <small>${fila.abreviatura}</small>
                    </div>
                    <div class="col-1 px-2 py-1">
...
function cargarCropsEntreLasFechas(initial_date, final_date, granjas = [], oncomplete = () => { }) {
	$.ajax({
		type: "POST",
		// url: window.location.origin + "/public/api/v1/reportes/obtenerCropsEntreFechasAsJson",
...
function cargarTodosLosCrops(oncomplete = () => { }) {
	$.ajax({
		type: "POST",
		// url: window.location.origin + "/public/api/v1/reportes/obtenerTodosLosCrops",
...
function exportarArchivo(initial_date, final_date, employees, farms, crops, formato = 'xlsx') {
	// Crear el formulario dinámicamente
	var form = $('<form>', {
		// action: window.location.origin + '/public/v1/empleado/exportPieceRateFile',  // URL del endpoint
		action: window.location.origin + '/v1/empleado/exportPieceRateFile',  // URL del endpoint
