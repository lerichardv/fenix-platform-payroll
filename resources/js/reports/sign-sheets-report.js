...
document.addEventListener("DOMContentLoaded", () => {
	...
	$('#search').on('keyup', function () {
		...
		$.ajax({
			// url: window.location.origin + "/public/api/v1/empleado_administracion/searchEmpleadosSign",
...
function generarReporte() {
	...
	$.ajax({
		type: "POST",
		// url: window.location.origin + "/public/api/v1/reportes/obtenerSignSheetReport",
...
function exportarArchivo(week, farms, locations, employeeCodes, employeeCategories, formato = 'xlsx') {
	...
	var form = $('<form>', {
		// action: window.location.origin + '/public/v1/empleado/exportSignSheetReport',  // URL del endpoint
